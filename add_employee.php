...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($full_name) || empty($email) || empty($password) || empty($role) || empty($shift_schedule)) {
    ...
    $chksql = "SELECT id FROM users WHERE email = ?";
    ...
    $sql = "INSERT INTO users (full_name, email, password_hash, role, profile_image_url, shift_schedule) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ssssss", $full_name, $email, $password_hash, $role, $profile_image_url, $shift_schedule);
